...
void enc(int rounds, uint8_t *state, uint8_t *key){
    uint8_t rconst = 0x00;
    uint8_t state_duplicate[32];
    for(int r=0; r<rounds; r++){
        if(r==35){
        	trigger_high();
        	for(int i=0;i<32;i++)
        		state_duplicate[i]=state[i];
        }
        SBox(state);
        PLayer(state);
        ...
        addRk(state, key);
        if(r>=35){
        	SBox(state_duplicate);
		PLayer(state_duplicate);
		addRc(state_duplicate, rconst);
		addRk(state_duplicate, key);
        }
        Key_update(key);
        if(r==35) trigger_low();
    }
    int match_flag=0;
    for(int i=0;i<32;i++)
        		if(state_duplicate[i]!=state[i]) match_flag=match_flag+1;
    if(match_flag!=0)
    	for(int i=0;i<32;i++)
        		state[i]=0;
